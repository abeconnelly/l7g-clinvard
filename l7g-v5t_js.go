...
func (lvcvd *LVCVD) status_otto(call otto.FunctionCall) otto.Value {
...
func (lvcvd *LVCVD) JSVMRun(src string) (rstr string, e error) {
  js_vm := otto.New()

  fmt.Printf("JSVM_run:\n\n")

  //init_js,err := ioutil.ReadFile("js/init.js")
  init_js,err := ioutil.ReadFile( lvcvd.JSDir + "/init.js")
  if err!=nil { e = err; return }
  js_vm.Run(init_js)

  js_vm.Set("status", lvcvd.status_otto)
  js_vm.Set("lvcvd_sql", lvcvd.sqlexec_otto)
